...
public class MainActivity extends AppCompatActivity {
    String url = "http://172.16.0.11:8084";
    ...
    void getReport() {
        ...
        Call<ArrayList<Weather>> call = service.getWheatherReport();

        call.enqueue(new Callback<ArrayList<Weather>>() {
            @Override
            public void onResponse(Response<ArrayList<Weather>> response, Retrofit retrofit) {

                try {
                    String city="ll" ;//= response.body().getName();

                    String status = response.body().get(4).getCorporateName();

                    String humidity="ll";// = response.body().getMain().getHumidity().toString();

                    String pressure="ll"; //= response.body().getMain().getPressure().toString();
